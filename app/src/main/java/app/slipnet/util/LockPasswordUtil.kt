...
object LockPasswordUtil {
    ...
    /**
     * Fixed key so a profile exported on one install can be imported on
     * another. The upstream build derived this inside a native library; this
     * build derives it with SHA-256 over a constant seed.
     */
    private const val CONFIG_KEY_SEED = "dnstun-config-v1"

    private val aesKey: SecretKeySpec by lazy {
        val keyBytes = MessageDigest.getInstance("SHA-256")
            .digest(CONFIG_KEY_SEED.toByteArray(Charsets.UTF_8))
