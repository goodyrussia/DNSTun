...
/**
 * The single tunnel this app ships: the dnstun stream engine against our own
 * server. Every other engine the upstream project carried (DNSTT, NoizDNS,
 * VayDNS, Slipstream, SSH, NaiveProxy, DoH, Tor, VLESS, SOCKS5) has been
 * removed from the source tree, so the type list is one entry.
 */
enum class TunnelType(val value: String, val displayName: String) {
    DNSTT("dnstt", "DNS Tunnel");

    companion object {
        fun fromValue(value: String): TunnelType = DNSTT
    }
}

fun TunnelType.isAvailable(): Boolean = true
