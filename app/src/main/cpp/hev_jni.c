 ...
 *
 * Adapted from SlipNet's wrapper (anonvector/SlipNet, GPL-3.0) which uses the
 * same patched hev API (main_from_str + quit + stats + reject_quic +
 * reject_non_dns_udp). Symbol names retargeted to com.dnstun.app.HevTunnel.
 *
 * The tunnel runs on its own thread; nativeStart returns immediately and
 * nativeStop() quits it and joins.
 */
...
Java_com_dnstun_app_HevTunnel_nativeStart(
    JNIEnv *env,
    jclass clazz,
    jstring config,
    jint tun_fd
...
Java_com_dnstun_app_HevTunnel_nativeStop(
    JNIEnv *env,
    jclass clazz
) {
    if (!tunnel_running) {
...
JNIEXPORT void JNICALL
Java_com_dnstun_app_HevTunnel_nativeSetRejectQuic(
    JNIEnv *env,
    jclass clazz,
    jboolean enabled
) {
...
JNIEXPORT void JNICALL
Java_com_dnstun_app_HevTunnel_nativeSetRejectNonDnsUdp(
    JNIEnv *env,
    jclass clazz,
    jboolean enabled
) {
...
Java_com_dnstun_app_HevTunnel_nativeIsRunning(
    JNIEnv *env,
    jclass clazz
) {
...
Java_com_dnstun_app_HevTunnel_nativeGetStats(
    JNIEnv *env,
    jclass clazz
) {
